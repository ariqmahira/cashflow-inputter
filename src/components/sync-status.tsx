    ...
    return (
      <div className="mb-4 rounded-card bg-teler-wash p-4">
        <p className="text-sm font-medium text-ink">
          {dead.length} catatan gagal dikirim
        </p>
        ...
        <p className="mt-2 text-xs text-ink-soft">
          Catatannya masih ada di HP ini tapi belum masuk server. Coba catat ulang.
        </p>
        <button
          type="button"
  ...
  if (waiting > 0) {
    return (
      <p className="mb-4 rounded-card bg-gula-wash px-4 py-2.5 text-xs text-ink-soft">
        {waiting} catatan nunggu koneksi. Aman — tersimpan di HP dulu.
      </p>
    );
  }
  ...
  if (state.stale) {
    return (
      <p className="mb-4 rounded-card bg-sunken px-4 py-2.5 text-xs text-ink-soft">
        Lagi nggak bisa nyambung. Ini data terakhir yang tersimpan.
      </p>
    );
  }
